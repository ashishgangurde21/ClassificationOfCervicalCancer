...
################################################KNN ALGORITHM#######################################################
pre_process <- preProcess(training_balanced, method = c("scale", "center"))
pre_process
...
###############################VISUALISE#################################
results <- resamples(list(SVM_Linear=svm_Linear_Grid,SVM_Radial=svm_Rad))
summary(results)
# boxplots of results
bwplot(results)
...
model <- train(Hinselmann~
                 ...
                 Schiller+
                 Biopsy+
                 Citology, data=training_balanced, method="lvq", preProcess="scale", trControl=control)
